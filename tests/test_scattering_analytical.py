...
def test_rectangular_under_30_degree():
    ...
    result = ik.scattering.analytical.rectangular(
        x*c_over_L, np.array([30]), 0.50, 1, 0.49, 343.901)
...
def test_rectangular_wrong_input():
    with pytest.raises(TypeError, match='frequency_vector'):
        ik.scattering.analytical.rectangular(
            np.array([2+3j]), np.array([30]), 0.5, 1, 0.49, 343.901)
    ...
    with pytest.raises(TypeError, match='phis'):
        ik.scattering.analytical.rectangular(
            np.array([500]), np.array([7+3j]), 0.5, 1, 0.49, 343.901)
    ...
    with pytest.raises(TypeError, match='width'):
        ik.scattering.analytical.rectangular(
            np.array([500]), np.array([30]), 0, 1, 0.49)
    ...
    with pytest.raises(TypeError, match='height'):
        ik.scattering.analytical.rectangular(
            np.array([500]), np.array([30]), 0.5, 1, -3, 343.901)

    with pytest.raises(TypeError, match='c'):
        ik.scattering.analytical.rectangular(
            np.array([500]), np.array([30]), 0.5, 1, 0.49, -343.901)
