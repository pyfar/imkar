...
def test_freefield_1(frequencies):
    ...
    s = scattering.coefficient.freefield(p_sample, p_reference, mics.weights)
...
def test_freefield_wrong_input(frequencies):
    ...
    with pytest.raises(ValueError, match='sample_pressure'):
        scattering.coefficient.freefield(1, p_reference, mics.weights)
    with pytest.raises(ValueError, match='reference_pressure'):
        scattering.coefficient.freefield(p_sample, 1, mics.weights)
    with pytest.raises(ValueError, match='weights_microphones'):
        scattering.coefficient.freefield(p_sample, p_reference, 1)
    with pytest.raises(ValueError, match='cshape'):
        scattering.coefficient.freefield(
    ...
    with pytest.raises(ValueError, match='same frequencies'):
        ...
        scattering.coefficient.freefield(p_sample, p_reference, mics.weights)
